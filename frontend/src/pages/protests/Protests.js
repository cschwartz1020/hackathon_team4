...
const Protests = () => {
  ...
  const getLocalProtests = async () => {
    ...
    axios
      .get(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${userCoord.lat},${userCoord.lng}&sensor=true&key=${process.env.REACT_APP_API_KEY}`
      )
      .then((res) => {
  ...
  useEffect(() => {
    ...
    const getProtests = async (userProtests) => {
      await axios
        .get("http://localhost:3000/api/protests/", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    <div className="protests">
      <div className="row">
        <div className="leftcolumn">
          <SimpleMap
            clickedMarker={{
              lat: protestClicked.startLocation[0].location.latitude,
              lng: protestClicked.startLocation[0].location.longitude,
              title: protestClicked.title,
            }}
            markers={markers}
          />
          <MarkerLegend />
        ...
        <div className="rightcolumn">
          <Flex justify="center" align="center">
            ...
            <Switch
              id="protest-type"
              size="lg"
              onChange={() => setOnlyLocalProtests(!onlyLocalProtests)}
              isChecked={onlyLocalProtests}
            />
          </Flex>
          {onlyLocalProtests
            ? localProtests.map((p) => (
